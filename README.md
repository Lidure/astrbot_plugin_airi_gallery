...
**功能：** 暗色/亮色模式切换（自动跟随系统）、紧凑图标分页、并发加载 + 指数退避重试、图片上传与删除、上传队列内按 Git blob SHA 去重、上传前只在目标分类内做远程精确/感知查重、按全局最大编号续号。GitHub 批量上传会以受限并发准备 blob，再把全部图片和 `gallery/gallery_index.json` 放进同一个原子 commit，只更新一次分支；网络抖动与限流只重试安全的读取/对象创建，ref 响应不确定时会重新核验。超过 GitHub 100 MiB 单文件限制的图片会在读取前明确拒绝；GitHub 单图达到 4 MiB 时改走同源 Cloudflare Worker 原始二进制流式代理，浏览器不再构造整张 Base64 大 JSON；含大图的批次串行创建 blob，降低内存峰值并改善跨境上传稳定性。多个浏览器撞到同一编号时保持 create-only 并 fail-closed，不覆盖已有图片，失败项会留在队列供同步后重试。Gitee 仍使用逐文件写入与补偿删除。v2.11.11 起页面脚本与样式完全外置，并启用 CSP；远程分类名等仓库数据使用安全 DOM API 渲染。
