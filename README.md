...
如果 `看全部` 生成的总览图太大，可以在配置里调整：

| 配置项 | 作用 | 建议 |
| :--- | :--- | :--- |
| `view_all_collage_compress` | 是否压缩看全部拼图 | `false` 保持原样，`true` 开启压缩 |
| `view_all_collage_scale` | 压缩比例 | 只在开启压缩时生效，建议 `0.5` 到 `1.0`，默认 `0.85` |

开启后，拼图整体尺寸会缩小，同时 PNG 保存也会使用更高压缩率。
...
| `view_all_collage_compress` | 是否压缩看全部拼图 | `true` 会缩小拼图并提高 PNG 压缩率 |
| `view_all_collage_scale` | 看全部拼图压缩比例 | `0.5` 更紧凑，`1.0` 接近原始大小 |
